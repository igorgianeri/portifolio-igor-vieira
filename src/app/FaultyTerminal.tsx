...
uniform vec3  uTint;
uniform vec3  uBg;                // ADDED: background base color
uniform vec2  uMouse;
...
function hexToRgb(input: string) {
  ...
  // rgb(...) or rgba(...)
  if (s.startsWith('rgb')) {
    const nums = s.replace(/rgba?\(|\)/gi, '').split(',').map(n => parseFloat(n.trim()));
    if (nums.length >= 3) {
      const r = Math.max(0, Math.min(255, nums[0]));
      const g = Math.max(0, Math.min(255, nums[1]));
      const b = Math.max(0, Math.min(255, nums[2]));
...
export default function FaultyTerminal({
  ...
  useEffect(() => {
    ...
    // make canvas transparent and let body gradient be visible under shader
